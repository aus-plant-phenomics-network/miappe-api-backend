...
class BaseController(Controller):
    def __init__(
        self, table: DeclarativeBase, dto_struct: "DTOGenerator", *args, **kwargs
    ):
        self.table = table

        @get("/{id:uuid}", return_dto=dto_struct.read_dto)
        ...
        @post(dto=dto_struct.write_dto, return_dto=dto_struct.write_dto)
        async def _create_item(self, transaction: "AsyncSession", data: table) -> table:
            return await create_item(session=transaction, data=data)

        @put("/{id:uuid}", dto=dto_struct.update_dto, return_dto=dto_struct.read_dto)
        async def _update_item(
            self, transaction: "AsyncSession", id: UUID, data: table
        ) -> table:
            result = await update_item(
                session=transaction, id=id, data=data, table=table
            )
            return result
