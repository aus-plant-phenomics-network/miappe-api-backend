...
from miappe.metadata.isatools.utils import utf8_text_file_open
from miappe.metadata.isatools.isatab.load import read_investigation_file
from miappe.metadata.isatools.isatab.defaults import _RX_COMMENT, default_config_dir, log
from miappe.metadata.isatools.isatab.validate.store import validator as message_handler
from miappe.metadata.isatools.isatab.validate.rules.core import (
...
def validate_mzml(fp: TextIO) -> None:
    ...
    :param fp: the investigation file handler
    """
    from miappe.metadata.isatools import utils
    try:
        fp.seek(0)
...
def batch_validate(tab_dir_list):
    ...
    Example:
        from miappe.metadata.isatools import isatab
        my_tabs = [
            '/path/to/study1/',
            '/path/to/study2/'
        ]
